...
export function generateSalesQuoteData(packageId, addonSelection, clientInfo, salesUser, { discountRate = 0 } = {}) {
  ...
  const scope = [
    {
      number: '1',
      title: `Web stranica · paket ${pkg.name}`,
      items: pkg.features,
    },
  ];
  ...
  return {
    ...
    project_overview: `Ova ponuda obuhvaća izradu profesionalne web stranice (paket ${pkg.name}) za ${clientRef}. Uključuje profesionalnu podršku i redovitu komunikaciju tijekom trajanja projekta.`,
    ...
    quote_data: {
      objectives: [
        'Izgraditi modernu i funkcionalnu web stranicu',
