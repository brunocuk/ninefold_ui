...
export function calculateQuoteFromSelections(selections) {
  ...
  if (selections.socialMedia?.enabled && selections.socialMedia?.plan) {
    ...
    const customPlan = selections.socialMedia.customPlan;

    // Get management price (custom or preset)
    const managementPrice = isCustomPlan
      ? (customPlan?.managementPrice || 0)
      : (plan?.managementPrice || 0);

    // Get plan name for display
    const planName = isCustomPlan ? 'Prilagođeni' : (plan?.nameHr || '');

    // Build description for custom plans with deliverables
    let managementDescription = '';
    if (isCustomPlan && customPlan?.contentDeliverables) {
      const deliverablesList = Object.entries(customPlan.contentDeliverables)
        .filter(([_, data]) => data.weekly > 0)
        .map(([contentId, data]) => {
          const contentType = CONTENT_TYPES[contentId];
          return `${data.weekly * 4}x ${contentType?.nameHr || contentId}`;
        });
      managementDescription = deliverablesList.length > 0
        ? deliverablesList.join(', ') + ' mjesečno'
        : 'Prilagođeni paket';
    } else if (plan) {
      managementDescription = `${plan.postsPerMonth} objava mjesečno`;
    }

    if (managementPrice > 0 || isCustomPlan) {
      // Management fee
      lineItems.monthly.push({
        category: 'socialMedia',
        name: `Social Media Management - ${planName}`,
        description: managementDescription,
        monthlyPrice: managementPrice,
        annualPrice: managementPrice * 12
      });
      monthlySubtotal += managementPrice;

      // Content production (separate from custom plan deliverables)
      ...
      Object.entries(contentQuantities).forEach(([contentTypeId, quantity]) => {
        if (quantity > 0) {
          ...
          if (contentType) {
            // For photo content, respect the plan's max photos limit (only for preset plans)
            let actualQuantity = quantity;
            if (contentTypeId === 'fotografija' && !isCustomPlan && plan?.maxPhotos) {
 ...
 * @param {string} issuerCompany - Company issuing the quote ('progmatiq' or 'endemik')
 * @returns {Object} - Quote record data
 */
export function generateQuoteData(selections, clientInfo, issuerCompany = 'progmatiq') {
...
function buildScopeFromSelections(selections) {
  ...
  if (selections.socialMedia?.enabled) {
    ...
    // Use custom features if custom plan, otherwise use preset features
    const features = isCustomPlan && customPlan?.features?.length > 0
      ? customPlan.features
      : (plan?.features || ['Upravljanje profilima', 'Community management', 'Mjesečni izvještaji']);
...
function buildProjectOverview(selections, clientInfo) {
  ...
  if (selections.socialMedia?.enabled) {
    const isCustomPlan = selections.socialMedia.plan === 'custom';
    const plan = SOCIAL_PLANS[selections.socialMedia.plan];
    const planName = isCustomPlan ? 'prilagođeni paket' : (plan?.nameHr || 'plan');
    parts.push(`upravljanje društvenim mrežama (${planName})`);
  }
...
export function validateSelections(selections) {
  ...
  if (selections.socialMedia?.enabled) {
    if (!selections.socialMedia?.plan) {
      errors.push('Odaberite social media plan');
    }
    // Validate custom plan requires management price
    if (selections.socialMedia.plan === 'custom') {
      if (!selections.socialMedia.customPlan?.managementPrice || selections.socialMedia.customPlan.managementPrice <= 0) {
        errors.push('Unesite mjesečnu cijenu za prilagođeni paket');
      }
    }
  }
